...
The algorithm was implemented in C++17 and compiled with \texttt{g++ -O3 -march=native}. For $n=11$ mode~0, the runtime dropped from 9.2~minutes to 8.5~seconds --- a $65\times$ speedup. Note that the $O(1)$ refers to the \emph{check} step (deciding whether a candidate point is blocked); the \emph{update} step after placing a point still requires $O(k)$ time to compute blocking lines from all $k$ existing points, so the overall per-placement cost is $O(k)$ rather than $O(1)$. Cross-validation against OEIS A000755~\cite{oeis} for $n\le 13$ confirmed correctness.
...
These patterns are observed in all five available data points ($n=12,14,16,18,72$) and persist at $n=72$, a scale $6\times$ larger than the next-largest analyzed ($n=18$). The orbit-to-ring mapping pattern is a consequence of C$_4$ orbit geometry rather than an independent empirical discovery. The limited sample size (only five $n$ with available detailed orbital data) precludes claims of universality, but the consistency across a $6\times$ scale range is suggestive.
...
\begin{table}[H]
\centering
\caption{Reconstructed full missing-center counts (values for $n\ge 14$ are reconstructed from $D_4$-inequivalent counts via the orbit multipliers, verified exact within 0.1\% sampling error).}
\label{tab:fullrecon}
\footnotesize
\begin{tabular}{crrcc}
\toprule
...
18 & 152210 & 2735 & 1.80\% & Even \\
19 & 258170 & 18729 & 7.25\% & Odd \\
20 & 941580 & 18326 & 1.95\% & Even \\
