...
\small \texttt{junrong.du@example.com}\\[4pt]
...
The evidence strongly supports $D(n)=2n$ for all even $n$. The sole remaining gap $\le 72$ is $n=71$, for which we propose a search strategy targeting $180^\circ$ rotational symmetry.
\end{abstract}
...
\item Evidence supporting $D(n)=2n$ for all even $n$, and a search strategy for the open case $n=71$.
\end{enumerate}
...
% ============================================================
\section{Status of $D(n)=2n$ and the Open Case $n=71$}
\label{sec:status}
...
\begin{enumerate}[nosep]
\item C$_4$-symmetric solutions exist for every even $n\le 72$ (Flammenkamp database).
\item The count of rot4 solutions grows exponentially ($\sim1.5\times$ per 2-step) with no slackening.
\item The structural argument via C$_4$ orbit theory guarantees constructibility at any even $n$.
\end{enumerate}

We conclude that $D(n)=2n$ for all even $n$ is effectively established.

\subsection{Odd $n$ and the $n=71$ Gap}
...
\subsubsection{Extinction of Missing-Center at $n\ge 33$}
...
\begin{table}[H]
\centering
\caption{Missing-center extinction for odd $n\ge 31$.}
\label{tab:extinct}
...
The threshold lies at a constraint density of approximately 74 triples per available pair. This is a classic SAT phase transition analogous to the well-known random $k$-SAT threshold. For $n\ge 33$, only rct4 solutions remain, and \textbf{rct4 solutions invariably have the grid center as a circumcenter}. This is a group-theoretic necessity: D$_4$ orbits on odd-$n$ grids force $\ge 4$ points per distance ring.

Thus, missing-center solutions are extinct for all odd $n\ge 33$. The center is always a circumcenter in every maximal $2n$-point solution above this threshold.
...
Our analysis suggests that $n=71$ must be targeted via the \textbf{rct4} symmetry class. All known solutions for odd $n\ge 33$, including Heule's $n=65,67,69$, are rct4. The rct4 class reduces the search to selecting fundamental-domain orbits (a polynomial reduction in variable count) and is the only viable structural class for large odd-$n$ grids.

\begin{conjecture}
$D(71)=2\times71=142$, and an rct4-symmetric solution exists. Any such solution necessarily has the grid center as a circumcenter.
\end{conjecture}
...
\item Complete missing-center counts for $n=2\dots13$ (full search) and $n=7\dots45$ ($D_4$-inequivalent), revealing a three-phase evolution: abundance ($n=7$--$19$), decline ($n=21$--$29$), and extinction ($n\ge33$).
...
\item Evidence that $D(n)=2n$ for all even $n$ and a revised search strategy for $n=71$ targeting rct4 symmetry.
\end{enumerate}
